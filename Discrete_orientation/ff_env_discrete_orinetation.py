...
class FFEnv(gym.Env):
    ...
    low = np.array([radians(0), radians(0), 0.5, 0.5, -2.0, -2.0, 0, -1])
    high = np.array([radians(180), radians(180), 2.5, 2.5, 2.0, 2.0, 1, +1])
    ...
    def __init__(self):
        ...
        self.ff_info = np.zeros(2, dtype=[('d', np.float32), ('t', np.float32), ('o', np.float32),('a', np.int)])
        self.goal = {'x': 0., 'y': 0., 'w':self.w0, 'o':0} # Goal Position of the Object 
        #self.goal['x'], self.goal['y'] = self.get_goal_point()
        self.goal['x'], self.goal['y'], self.goal['o'] = 388, 228, 90
        ...
        self.ff_info['d'][0] = 50               # Initialising dl in mm
    ...
    def step(self, action):
        ...
        # Rotate clockwise
            ...
            print("t0 : ", self.t0, " t1 : ", self.t1, "d0 : ", self.d0, " d1 : ", self.d1 )

            # self.d0 = self.d0 + 20#self.w0
            # self.d1 = self.d1 - 20#self.w0
            # t0f = np.pi - np.arccos((((self.d1 - self.w0)**2 + (self.w0+2*self.fw)**2 - self.wp**2 - (self.d0 + self.w0)**2)/(2 * self.wp * (self.d0 + self.w0))))

            # # print("Before right config d1 : ", self.d1)
            # t1f, d1_ = self.calc_right_config(t0f, self.d0)
            # # print("After right config d1 : ", d1_)           
            # self.t1 = t1f
            # self.t0 = t0f
            ...
            # elif (action_flag == True): # Take action
            #     print("Action of orienting object clockwise is taken")
                
            #     self.ff_info['t'][1] = self.t1   
            #     self.ff_info['t'][0] = self.t0
            #     self.ff_info['d'][1] = self.d1
            #     self.ff_info['d'][0] = self.d0
            #     self.ff_info['o'][0] += 90
            #     print("t0 : ", self.t0, " t1 : ", self.t1, "d0 : ", self.d0, " d1 : ", self.d1 )
            #     print(self.ff_info)
        ...
        # Rotate anti-clockwise
            ...
            # self.d1 = self.d1 + 20#self.w0
            # self.d0 = self.d0 - 20#self.w0
            # t1f = np.arccos(((self.d0 - self.w0)**2 + (self.w0 + 2*self.fw)**2 - (self.d1 + self.w0)**2 - self.wp**2)/(2*self.wp*(self.d1 + self.w0)))

            # t0f, d0_ = self.calc_left_config(t1f, self.d1)
            # self.t1 = t1f
            # self.t0 = t0f
            ...
            # elif (action_flag == True): # Take action
            #     # print("Action of orienting object counter clockwise is taken")
        ...
        # reward engineering for distance
        reward_1 = -np.sqrt((dist_x**2 + dist_y**2))/200
        # print("reward_1 : ", reward_1)
        # reward engineering for orientation
        reward_2 = -1* (abs(self.goal['o'] - self.ff_info['o'][0]))/180
