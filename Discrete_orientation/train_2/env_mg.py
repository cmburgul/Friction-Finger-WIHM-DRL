...
class Friction_finger_env(gym.Env):
    ...
    def __init__(self):
        ...
        action_table_generate = False # Boolean to generate action_table
        object_s = 1.5
        low_limit = 8.0
        high_limit = 10.0
        start=(8.0,8.0,0)
        ...
        if 1:
            ...
            # if 1:
                if action == 0:
                    ...
                    return(round(self.current_state[0],10),round(self.current_state[1],10),self.current_state[2])
            else:
    ...
    def calculate_reward(self,action,next_state):   ## Remember to make changes in the compute function reward below also to reflect the changes
        if(self.goal[0]- 0.5 < self.current_state[0] < self.goal[0] + 0.5
           and self.goal[1]- 0.5 < self.current_state[1] < self.goal[1]+0.5
           and self.current_state[2] == self.goal[2]):
            # print(self.next_state,self.goal)
            return 0
        else:
            return -math.sqrt(math.pow((self.current_state[0]-self.goal[0])*10,2)+math.pow((self.current_state[1]-self.goal[1])*10,2))/2-(abs(self.current_state[2]-self.goal[2])/90)
            # return -1
    ...
    def reset(self):
        ...
        self.goal = (random.randint(self.finger_low_limit*10,self.finger_high_limit*10)/10.0,random.randint(self.finger_low_limit*10,self.finger_high_limit*10)/10.0, 0)
